...
interface DarkModeConfig {
  readonly defaultDarkFactor: number;
  readonly maxDarkFactor: number;
  readonly depthDarkFactorIncrement: number;
  readonly interactiveDarkFactor: number;
  readonly cardDarkFactor: number;
}

// 設定値
const DARK_MODE_CONFIG: DarkModeConfig = {
...
class ColorUtils {
  ...
  static calculateBrightness({ r, g, b }: RGBColor): number {
    return (r * 299 + g * 587 + b * 114) / 1000;
  }

  static isLightBackground(color: string): boolean {
    const rgb = this.parseRGB(color);
    if (!rgb) return false;
    return this.calculateBrightness(rgb) > 128;
  }

  static applyDarkFactor({ r, g, b }: RGBColor, darkFactor: number): string {
...
// 要素処理ユーティリティ
class ElementUtils {
  static getElementDepth(element: HTMLElement): number {
    let depth = 0;
    let parent = element.parentElement;
    while (parent) {
      if (parent.tagName === element.tagName) depth++;
      parent = parent.parentElement;
    }
    return depth;
  }

  static getDarkFactor(element: HTMLElement): number {
    if (element.tagName === 'TABLE') {
      const depth = this.getElementDepth(element);
      return Math.min(
        DARK_MODE_CONFIG.defaultDarkFactor + 
        (depth * DARK_MODE_CONFIG.depthDarkFactorIncrement),
        DARK_MODE_CONFIG.maxDarkFactor
      );
    }
  ...
  static adjustTextColor(element: HTMLElement): void {
